      ...
      <main className="flex-1 flex flex-col p-10 text-white bg-slate-300">
        <div className="w-3/4 mt-20">
          <div className="w-full flex justify-between">
            ...
            <span>{language === 'es' ? 'Inventario Material' : 'Material Inventory'}</span>            </h1>
          </div>
        </div>
          ...
          <div className="relative">
            ...
            {showTooltip === 'add' && (
              <div className="absolute bottom-full mb-2 left-1/2 transform -translate-x-1/2 px-4 py-2 bg-purple-600 text-white text-lg rounded shadow-lg z-10">
                {language === 'es' ? 'Registrar Nuevo':'Actions'}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex justify-between items-center p-4 bg-gray-900 text-white">
            ...
            {language === 'es'  
             ? `Mostrando {currentEmpleados.length} de {filteredEmpleados.length} empleados`
             : `Showing ${currentEmpleados.length} of ${filteredEmpleados.length} employees`}
            </span>
          ...
          <table className="w-full bg-gray-900">
            <thead className="bg-purple-600 text-white">
              <tr>
                <th className="p-3 text-center">{language === 'es' ? 'Nombre':'Name'}</th>
                <th className="p-3 text-center">{language === 'es' ? 'Tipo Documento':'Actions'}</th>
                <th className="p-3 text-center">{language === 'es' ? 'Fecha de nacimiento':''}</th>
                <th className="p-3 text-center">{language === 'es' ? 'Correo':'Actions'}</th>
                <th className="p-3 text-center">{language === 'es' ? 'Celular':'Actions'}</th>
                <th className="p-3 text-center">{language === 'es' ? 'Acciones':'Actions'}</th>
              </tr>
            </thead>
